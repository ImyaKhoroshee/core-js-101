...
function isInsideCircle(circle, point) {
  return (circle.radius === point.y && circle.radius === point.x)
...
function isCreditCardNumber(ccn) {
  ...
  const temp = Array.from(String(ccn), Number).reverse();
  for (let i = 1; i < temp.length; i += 2) {
    temp[i] *= 2;
  }
  const afterMult = temp.join('');
  Array.from(afterMult, Number).forEach((elem) => { sum += elem; });
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  while (temp > 9) {
    const numArray = Array.from(String(temp), Number);
    temp = numArray.reduce((a, b) => a + b);
  }
  return temp;
}
...
function isBracketsBalanced(str) {
  const listOfBracketsAndEmply = ['[]', '()', '{}', '<>', ' '];

  if (listOfBracketsAndEmply.includes(str)) {
    return true;
  }
  const strToArr = str.split('');

  const openBracketsAndDigits = ['(', '[', '{', '<'];
  const closedracketsAndDigits = [')', ']', '}', '>'];
  const sequence = ['[]', '{}', '()', '<>'];
  ...
  strToArr.forEach((symbol) => {
    ...
    if (closedracketsAndDigits.includes(symbol)) {
      resultArray.push(symbol);
      if (sequence.includes(resultArray.slice(-2).join(''))) {
...
function getCommonDirectoryPath(pathes) {
  const minLengthPath = pathes.sort((a, b) => b.length - a.length).pop().split('/');

  const tempArr = [];

  for (let i = 0; i < pathes.length; i += 1) {
    tempArr.push(pathes[i].split('/'));
  }
  ...
  while (commonDirPath === '') {
    for (let j = 0; j < minLengthPath.length; j += 1) {
      for (let k = 0; k < tempArr.length; k += 1) {
        if (minLengthPath[end] !== tempArr[k][end]) {
          if (minLengthPath[end - 1] !== '' && tempArr[k][end - 1] === '') {
            commonDirPath += '';
      ...
      commonDirPath += `/${minLengthPath[end]}`;
      end += 1;
    }
  }
  return commonDirPath;
...
function getMatrixProduct(m1, m2) {
  ...
  for (let a = 0; a < m1.length; a += 1) {
    for (let b = 0; b < m1.length; b += 1) {
      let production = 0;

      for (let c = 0; c < m2.length; c += 1) {
        production += m1[a][c] * m2[c][b];
      }
      result[a].push(production);
    }
  }
  return result;
}
